...
> 4. 형제 노드에 여유가 없으면 가운데 키를 부모 노드로 넘기고 노드를 두 개로 분리한다. 분리 작업은 부모 노드에서의 삽입 작업을 포함한다.
...
- 언더플로가 발생할 때는 우선 키를 가져올 수 있는 형제 노드가 있는지 본다. 그런 노드가 있으면 가져다 채우고 끝낸다. 그렇지 않으면 형제 노드와 병합을 해야 한다. 병합은 두 노드를 하나로 합치는 것이므로 부모 노드의 키 중 하나가 필요 없다. 이 필요 없는 키와 두 노드를 합쳐 하나의 노드로 만든다. 이 병합의 결과키가 하나 줄어든 부모 노드에서 언더플로가 발생할 수 있다. 이를 재귀적으로 처리할 수 있다.
...
### 🎈 B-트리의 작업 성능 분석
- 이진 검색 트리가 균형을 아주 잘 맞추면 높이가 log<sub>2</sub>n에 근접할 수 있다. 
