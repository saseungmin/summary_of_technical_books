...
버그가 여러분의 잘못인지 다른 사람의 잘못인지는 중요치 않다. 어쨌ㄷ거나 그 버그를 해결해야 하는 사람은 여러분이다.
...
- 처음에 받은 자료 이상을 얻기 위해서 버그를 보고한 사용자를 인터뷰할 필요도 있따.
...
까끔은 영원히 스크롤 해야 할 것 같은 긴 스택 트레이스를 만날 때가 있다. 이런 때는 스택 프레임의 일일이 조사하는 것보다 더 빠른 방법이 있다. 바로 "이진 분할"을 하는 것이다.
...
## 🍭 엔지니어링 일지
우리도 일지를 사용한다. 회의에서 메모할 때나 작업하는 내용을 써 놓을 때, 디버깅하다가 변수의 값을 적어 놓을 때, 무엇을 어디 두었는지 기록을 남길 때, 엉뚱한 생각을 기록할 떄, 아니면 때로는  그냥 낙서를 할 때 일지를 쓴다. 일지를 쓰면 좋은 점이 크게 세 가지 있다.
