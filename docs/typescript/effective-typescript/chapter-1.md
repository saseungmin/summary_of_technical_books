...
타입스크립트는 문법적으로도 자바스크립트의 상위집합(superset)입니다. 자바스크립트 프로그램에 문법 오류가 없다면, 유효한 타입스크립트 프로그램이라고 할 수 있습니다. 타입스크립트는 자바스크립트의 상위집합이기 때문에 `.js` 파일에 있는 코드는 이미 타입스크립트라고 할 수 있습니다. `main.js` 파일명을 `main.ts`로 바꾼다고 해도 달라지는 것은 없습니다.   
