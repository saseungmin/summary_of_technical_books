...
### 🎈 구조체 감추기
만약 `ctxt`, `options`, `scratchDir`이 진짜 객체라면? 앞서 코드 예제처럼 줄줄이 사탕으로 엮어서는 안 된다. 객체라면 내부 구조를 감춰야 하니까. 그렇다면 임시 디렉터리의 절대 경로는 어떻게 얻어야 좋을까?
...
`ctxt`가 객체라면 **뭔가를 하라고** 말해야지 속을 드러내라고 말하면 안 된다. 다음은 같은 모듈에서 가져온 코드다.
